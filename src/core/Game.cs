...
namespace GameOfLife
{
    public class Game
    {
        ...
        public void SetSettlement(bool[,] settlement, Graphics2D.Location location)
        {
        ...
        public static void GeneratePopulation(Game game, int count = -1, params bool[][,] templates)
        {
            ...
            {
                var template = templatecount > 0 ? templates.ElementAt(rdm.Next(templatecount)) : Graphics2D.Settlement.Ring3x3;

                var location = new Graphics2D.Location(rdm.Next(game.Width), rdm.Next(game.Height));
