...
function Get-PublishKey([string]$uploadUrl) {
  ...
  switch ($url.Host) {
    "YourURLToOneOrMoreNugetFeeds" { return $nugetApiKey }
    default {
       write-warning "Consider adding the the URL to the list of allowed feeds.\nGo to publish-assets.ps1. search for Get-Publishkey and add your source to the >Case< statment."
       throw "Cannot determine publish key for $uploadUrl" 
      }
  }
}
